...
This package is build on top of C<Loggator>, a project (very much
...
=item * no support for user->VO mapping (in progress)

=item * the so-called undo files are really just fake sql dumps
...
logs and gridftpd logs. While they have absolutely nothing to do, you
might want to take a look at them and use the Loggator infrastructure
for a different application. Good luck ;-)
